        ...
        for i in range(self.config.num_hidden_layers):
            ...
            if getattr(self.config, "gradient_checkpointing", False) and self.training:
                ...
                layer_outputs = layer_module(
                    hidden_states, # [2,64,10224] , learnable queries
                    attention_mask,
                    layer_head_mask,
                    encoder_hidden_states, # image embeddings
                    encoder_attention_mask,
                    output_attentions,
                )
...
class MplugOwlVisualAbstractorModel(PreTrainedModel):
    ...
    def __init__(self, config, language_hidden_size,num_learnable_queries=None):
        super().__init__(config)
        self.config = config
        ...
        if num_learnable_queries is None:
            num_learnable_queries = config.num_learnable_queries
        self.query_embeds = torch.nn.Parameter(torch.randn(1, num_learnable_queries, config.hidden_size))
    ...
    def forward(
        ...
        query_embeds = self.query_embeds.repeat(encoder_hidden_states.shape[0], 1, 1) # bs, config.num_learnable_queries, config.hidden_size
