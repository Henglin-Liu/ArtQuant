...
class MPLUGOwl2MetaModel:
    def __init__(self, config):
        ...
        if "multi_scale" in config.keys():
            self.multi_scale = config.multi_scale
        else:
            self.multi_scale = False
            
        if "multi_image" in config.keys():
            self.multi_image = config.multi_image
        else:
            self.multi_image = False
            
        if self.multi_image:
            self.vision_model = nn.ModuleDict({
                "low": MplugOwlVisionModel(
                    MplugOwlVisionConfig(**config.visual_config["visual_model"])
                ),
                "mid": MplugOwlVisionModel(
                    MplugOwlVisionConfig(**config.visual_config["visual_model"])
                ),
                "high": MplugOwlVisionModel(
                    MplugOwlVisionConfig(**config.visual_config["visual_model"])
                )
            })
        else:
        ...
        if self.multi_scale or self.multi_image: # multi encoder need multi proj
            self.visual_abstractor = nn.ModuleDict({
                "low": MplugOwlVisualAbstractorModel(
                    MplugOwlVisualAbstractorConfig(**config.visual_config["visual_abstractor"]),
                    config.hidden_size,
                ),
                "mid": MplugOwlVisualAbstractorModel(
                    MplugOwlVisualAbstractorConfig(**config.visual_config["visual_abstractor"]),
                    config.hidden_size,
                ),
                "high": MplugOwlVisualAbstractorModel(
                    MplugOwlVisualAbstractorConfig(**config.visual_config["visual_abstractor"]),
                    config.hidden_size,
                )
            })
    ...
    def get_visual_abstractor(self,encoder_hidden_states):
        if self.multi_scale:
            image_features =  []
            for scale in ["low", "mid", "high"]:
                image_features.append(self.visual_abstractor[scale](encoder_hidden_states=encoder_hidden_states).last_hidden_state)
            image_features = torch.cat(image_features, dim=1)
            return image_features
        else:
            return self.visual_abstractor(encoder_hidden_states=encoder_hidden_states).last_hidden_state
...
class MPLUGOwl2MetaForCausalLM(ABC):
    ...
    def encode_images(self, images):
        save_attn = False
        if save_attn == True:
            output = self.get_model().vision_model(images)
            image_features = output.last_hidden_state
            image_features = (
                self.get_model()
                .get_visual_abstractor(encoder_hidden_states=image_features)
            )
            return image_features, output # ['last_hidden_state', 'pooler_output', 'hidden_states', 'attentions']
        else:
            image_features = self.get_model().vision_model(images).last_hidden_state # [2,1025,1024]
            image_features = (
                self.get_model()
                .get_visual_abstractor(encoder_hidden_states=image_features)
            )
    ...
    def encode_images_by_each_encoder(self,images):
        # images:[2,3,3,448,448]
        images = images.transpose(1,0) # [3,2,3,448,448]
        images_features = []
        for image,scale in zip(images,['low','mid','high']):
            if isinstance(self.get_model().vision_model, nn.ModuleDict):
                image_features = self.get_model().vision_model[scale](image).last_hidden_state
            else:
                image_features = self.get_model().vision_model(image).last_hidden_state
            images_features.append(
                self.get_model().visual_abstractor[scale](encoder_hidden_states=image_features).last_hidden_state
            ) # [2,65,4096] x 3
        images_features = torch.cat(images_features, dim=1) 
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if type(images) is list or images.ndim == 5:
            if isinstance(self.get_model().vision_model, nn.ModuleDict) or isinstance(self.get_model().visual_abstractor, nn.ModuleDict):
                image_features = self.encode_images_by_each_encoder(images) # [2,195,4096]
            else:
                concat_images = torch.cat([image for image in images], dim=0) # [2,3,448,448],mult:[2,3,3,448,448] -> [6,3,448,448]
                image_features = self.encode_images(concat_images) # mult:[6, 65, 4096]
                split_sizes = [image.shape[0] for image in images] # mult:[3,3]
                image_features = torch.split(image_features, split_sizes, dim=0) # [3, 65, 4096]
                image_features = [x.flatten(0, 1) for x in image_features] # mult:[3*65, 4096]
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            while image_token_indices.numel() > 0:
                ...
                cur_modality_indicators.append(
                    torch.ones(len(cur_image_features)).long() # 图像占65个token位置
                )
...
class MPLUGOwl2LlamaForCausalLM(LlamaForCausalLM, MPLUGOwl2MetaForCausalLM):
    ...
    def __init__(self, config):
        ...
        # self.tokenizer =  AutoTokenizer.from_pretrained('./preprocessor/', use_fast=False)
    ...
    def forward(self, input_type=None,attention_mask=None,past_key_values=None, **kwargs):
        if input_type is None:
            return self.forward_single(attention_mask=attention_mask,
    ...
    def softkl_loss(self, logits, labels, level_probs):
        ...
        for level_id in level_ids_label:
            assert level_id in self.config.level_ids, f"level_id {level_id} not in {self.config.level_ids}"
    ...
    def get_score(self, item):
        ...
        weights = torch.tensor(self.config.score_weight).to(probs) # [10.,7.5,5,3.5,1.0]
        ...
        return scores, stds, outputs.loss, loss_in_level # loss_in_level用来约束概率和为1，outputs.loss用来约束下一个token的预测
