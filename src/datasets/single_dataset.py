...
class SingleDataset(Dataset):
    ...
    def __init__(
        ...
        for data_path, data_weight in zip(data_paths, data_weights):
            ...
            if data_args.sample_per_dataset > 0:
                assert len(data_weights) == 1, "Sampling is not supported for multiple datasets"
                print(f"sampling data to {data_args.sample_per_dataset}")
                data_dict = random.sample(data_dict, data_args.sample_per_dataset)
            else:
                assert data_args.sample_per_dataset == -1
                if data_weight > 0:
                    upper_data_weight = int(data_weight)
                    sample_data_weight = data_weight - upper_data_weight
                    data_dict = data_dict * upper_data_weight + random.sample(data_dict, int(len(data_dict) * sample_data_weight))
                else:
                    raise ValueError("data_weight should be greater than 0")
            list_data_dict += data_dict
            print(f"Loaded {len(data_dict)} data from {data_path}")
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        while True:
            try:
                ...
                data_dict["level_probs"] = sources_org[0].get("level_probs", [-10000] * self.data_args.len_level_names)

                # image exist in the data
                if "image" in sources_org[0]:
                    data_dict["image"] = image # mult:[3, 3, 448, 448]
                ...
                return data_dict # mult:data_dict['image'].shape = [3, 3, 448, 448]
            except Exception as ex:
                print(ex)
                i = self.next_rand()
                continue
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if "image" in instances[0]:
            ...
            if all(x is not None and x.shape == images[0].shape for x in images):
                batch["images"] = torch.stack(images) # torch.Size([2, 3, 3, 448, 448])
            else:
                batch["images"] = images
